...
export default function OrderPage() {
  ...
  if (successMessage) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <h1 className="mt-4 text-xl font-semibold text-amber-900">{successMessage}</h1>
        <p className="mt-2 text-amber-700">Cảm ơn bạn đã đặt hàng. Chúng tôi sẽ liên hệ sớm nhất!</p>
        <Link
          href="/"
  ...
  if (items.length === 0) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        ...
        <p className="mt-2 text-amber-700">Bạn chưa có sản phẩm nào để đặt hàng.</p>
        <Link
          href="/"
          className="mt-6 inline-block bg-amber-600 hover:bg-amber-700 text-white font-medium px-6 py-2.5 rounded-lg transition-colors"
        >
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-amber-900 mb-6">Đặt hàng</h1>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Order Form */}
        <div className="lg:col-span-2">
          <form onSubmit={handleSubmit} className="bg-white border border-amber-200 rounded-lg p-6 space-y-5">
            {errorMessage && (
            ...
            {/* Customer Name */}
            <div>
              <label htmlFor="customerName" className="block text-sm font-medium text-amber-900 mb-1">
              ...
              <input
                ...
                className={`w-full px-3 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400 ${
                  errors.customerName ? "border-red-400" : "border-amber-200"
                }`}
            ...
            {/* Facebook Link */}
            <div>
              <label htmlFor="facebookLink" className="block text-sm font-medium text-amber-900 mb-1">
                Link Facebook
              </label>
              <input
                id="facebookLink"
        ...
        <div className="lg:col-span-1">
          <div className="bg-white border border-amber-200 rounded-lg p-6 sticky top-4">
            <h2 className="text-lg font-semibold text-amber-900 mb-4">Đơn hàng của bạn</h2>

            <div className="space-y-3 max-h-64 overflow-y-auto">
              {items.map((item) => (
                <div key={item.productId} className="flex items-center gap-3">
                  <div className="relative w-12 h-12 flex-shrink-0 rounded overflow-hidden bg-amber-100">
                    {item.thumbnailUrl ? (
                      <Image
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-amber-900 truncate">{item.productName}</p>
                    <p className="text-xs text-amber-600">x{item.quantity}</p>
                  </div>
            ...
            <div className="mt-4 pt-4 border-t border-amber-200 space-y-2">
              ...
              <div className="border-t border-amber-200 pt-2 flex justify-between text-base font-bold text-amber-900">
                <span>Tổng</span>
